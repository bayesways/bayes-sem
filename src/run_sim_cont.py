...
parser.add_argument(
    "-odr", "--off_diag_residual", help="off_diag_residual", type=bool, default=False
)
parser.add_argument("-gd", "--gen_data", help="gen fresh data", type=int, default=1)
parser.add_argument(
    "-off", "--standardize", help="standardize the data", type=int, default=1
)
parser.add_argument(
    "-rho", "--rho_param", help="off diag correlation of Theta", type=float, default=0.1
)
parser.add_argument(
...
parser.add_argument(
    "-c", "--c_param", help="fixed variances of Theta", type=float, default=1
)
parser.add_argument(
    "-cl_level",
    "--crossloading_level",
    help="level of cross loading magnitude",
    type=float,
    default=4,
)
...
if args.existing_directory is None:
    ...
    log_dir = (
        "./log/"
        + nowstr
        + "%s_s%sm%s/" % (args.task_handle, args.sim_case, args.stan_model)
    )
...
if args.gen_data == 1:
    ...
    if args.sim_case == 0:
        data = gen_data(
            args.nsim_data, off_diag_residual=False, random_seed=args.random_seed
        )
...
if args.compile_model == 0:
    ...
    try:
        ...
        file = open("log/compiled_models/cont/model%s/model.txt" % args.stan_model, "w")
        file.write(model_code)
        file.close()
    except:
