...
def compute_D(data, ps, mcmc_iter, pred=True):
    ...
    if pred == True:
        y_pred = multivariate_normal.rvs(
            mean=ps["alpha"][mcmc_iter],
            cov=ps["Marg_cov"][mcmc_iter],
            size=data["yy"].shape[0],
        )
        return ff2(y_pred, ps["alpha"][mcmc_iter], ps["Marg_cov"][mcmc_iter], p=J)

    else:
        return ff2(data["yy"], ps["alpha"][mcmc_iter], ps["Marg_cov"][mcmc_iter], p=J)
...
def variogram_score(y_pred, y_obs, p=0.5):
    ...
    for i in range(y_obs_length):
        for j in range(y_obs_length):
            ...
            s2 = (-1.0 / mcmc_lenght) * np.array(
                [np.abs(y_pred[k, i] - y_pred[k, j]) ** p for k in range(mcmc_lenght)]
            ).sum()
            s_ij = (s1 + s2) ** 2
            s = s + s_ij
    return s
...
def get_log_score(ps, data, nsim=10):
    mcmc_length = ps["alpha"].shape[0]
    if nsim > mcmc_length:
    ...
    for i in range(test_size):
        for m_ind in range(nsim):
            ...
            mean = ps["alpha"][m]
            Cov = ps["Marg_cov"][m]
            theta_draws[m_ind] = multivariate_normal.pdf(data[i], mean=mean, cov=Cov)
...
def get_energy_scores(ps, data, nsim=10, method_num=2):
    ...
    # use posterior median and log
        ...
        scores = -1.0 * Nlogpdf(data, m_alpha, m_Cov).sum()
    else:
        print("method_num not found")
    scores = {}
    scores_variogram = variogram_score_vector(y_pred=post_y, y_obs_vector=data).sum()
