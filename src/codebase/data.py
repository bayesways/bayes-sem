...
def gen_data(
    ...
    alpha = np.zeros(J)
    if cross_loadings:
        if cross_loadings_case == "b":
            rcl = norm.rvs(loc=0, scale=0.1, size=6)  # random_cross_loadings
            beta = np.array(
                [
                    [1, rcl[0]],
                    [b, rcl[1]],
        ...
        beta = np.array([[1, 0], [b, 0], [b, 0], [0, 1], [0, b], [0, b]], dtype=float)
    ...
    data["cross_loadings_case"] = cross_loadings_case
...
def gen_data_binary(
    ...
    if cross_loadings:
        if cross_loadings_level == 0:
            beta = np.array(
                [[1, 0.2], [b, -0.3], [b, -0.05], [-0.2, 1], [-0.08, b], [0.15, b]],
                dtype=float,
            )
        elif cross_loadings_level == 1:
            beta = np.array(
                [[1, 0], [b, 0], [b, 0.4], [0, 1], [0.4, b], [0, b]], dtype=float
            )
...
def gen_data_binary_1factor(
    nsim_data, J=6, K=1, c=1, noise=False, cheaters=False, random_seed=None
):
    if random_seed is not None:
        np.random.seed(random_seed)
    ...
    alpha = np.zeros(J)
    beta = np.array([1, 0.7, 0.8, 0.5, 0.9, 0.6])
...
def corr_headers(colnames):
    ...
    dfcols = list(
        zip(
            colnames[np.triu_indices(colnames.shape[0], k=1)[0]],
