...
model {
  ...
  to_vector(beta_zeros) ~ normal(0, 1);
  to_vector(beta_zeros) ~ normal(0, 0.1);
  to_vector(alpha) ~ normal(0, 1);
  sigma ~ cauchy(0,2);
  V_corr_chol ~ lkj_corr_cholesky(2);
  for (n in 1:N){
      to_vector(zz[n, ]) ~ multi_normal_cholesky(rep_vector(0, K), V_corr_chol);
  }
...
generated quantities{
  matrix [K, K] V = multiply_lower_tri_self_transpose(V_corr_chol);
