...
parameters {
  ...
  matrix[N,K] zz;
  matrix[N,J] uu;
  cholesky_factor_corr[J] L_R_u;
  vector<lower=0>[J] sigma_u;
}
...
model {
  ...
  L_R_u ~ lkj_corr_cholesky(2);
  sigma_u ~ cauchy(0,2);
  for (n in 1:N) to_vector(zz[n,])  ~ multi_normal_cholesky(zeros_K, L_R);
  for (n in 1:N) to_vector(uu[n,]) ~ multi_normal_cholesky(zeros_J, diag_pre_multiply(sigma_u, L_R_u));
