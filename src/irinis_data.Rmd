...
final_item_ids <- c(sample(clean_items, 10), sample(flagged_items, 10))
...
```{r}
df<- dfres %>% left_join(item_flag_df, by='item_id') %>%
  arrange(subj_flag, item_flag)


subj_order_df <- df %>% distinct(subj_id)
subj_order
```
...
final_response_df <- df %>%
  arrange(subj_flag, item_flag) %>%
  select(subj_id, item_id, res) %>%
  spread(item_id, res)
write_csv(final_response_df, "../dat/clean_sample_iri_data.csv")
...
write_csv(final_flag_df, "../dat/clean_sample_iri_flag_data.csv")
head(final_flag_df)
```
